(ns hospitalityops.operation
  "OperationActor -- one accommodation operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (HospitalityOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Hospitality Governor (:govern)
  ...
  One graph run = one accommodation operation (intake -> advise ->
  ...
  decision to a human operator (accommodation operator/front-desk
  manager). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:stay/check-in`/`:stay/check-out`
  ALWAYS reach this node when the governor is clean -- see
  `hospitalityops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [hospitalityops.hospitalityopsllm :as hospitalityopsllm]
...
(defn build
  ...
  `hospitalityops.store/Store`).
  opts:
    :advisor      -- a `hospitalityops.hospitalityopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (hospitalityopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; HospitalityOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (hospitalityopsllm/-advise advisor store request)]
            {:proposal p :audit [(hospitalityopsllm/trace request p)]})))

      ;; Hospitality Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
